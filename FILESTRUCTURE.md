...
* C++ source files:
  * `include/devdat/devdat.hpp` is the main
    C++ header that declares the interface of your library.
  * `app/devdat_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/devdat_t.cpp` contains the unit tests for the library.
